<template>
	<div>
		<el-page-header @back="back" content="客房具体信息" style="font-weight: bolder;line-height: 70px;">
		</el-page-header>
		 <el-descriptions
		 style="width: 65vw"
		 class="margin-top" 
		 :column="1" border
		 v-for="(idlist) in room">
		    <template slot="extra">
		     <el-button
		     type="primary" 
		     size="medium" 
...
<script>
	...
	export default {
	 ...
	 async created() {
	  ...
	  let res=await axios.get('/api/room/id',{params:{id: this.$route.params.id}}).then(res => {
	    this.data = res.data
	  }).catch(() => {
